...
async def create_bookmark(bookmark: BookmarkCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if not bookmark.url.startswith("http"):
        bookmark.url = "https://" + bookmark.url
    new_bookmark = Bookmark(url = bookmark.url, tags = bookmark.tags, user_id = current_user.id)
    db.add(new_bookmark)
    db.commit()
...
def search_bookmarks(q: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def get_bookmark(bookmark_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def update_bookmark(bookmark_id: int, updates: BookmarkUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    bookmark = db.query(Bookmark).filter(Bookmark.id == bookmark_id).first()
    if not bookmark or bookmark.user_id != current_user.id:
        raise HTTPException(status_code = 404, detail = "Bookmark not found")
